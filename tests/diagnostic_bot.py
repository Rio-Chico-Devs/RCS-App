...
def _mc(dia=None, lun=None, giri=None, spes=None, costo=None, conica=False):
    mc = MaterialeCalcolato()
    mc.diametro = dia if dia is not None else round(random.uniform(30, 300), 1)
    mc.lunghezza = lun if lun is not None else round(random.uniform(50, 500), 1)
    mc.giri = giri if giri is not None else random.randint(1, 8)
    mc.spessore = spes if spes is not None else round(random.choice([0.1,0.15,0.2,0.25,0.3,0.4,0.5]), 2)
    mc.costo_materiale = costo if costo is not None else round(random.uniform(5, 100), 2)
    if conica:
        mc.is_conica = True
        mc.conicita_lato = random.choice(["sinistro","destro"])
        mc.conicita_lunghezza_mm = round(random.uniform(10,100), 1)
        mc.conicita_altezza_mm = round(random.uniform(1,20), 1)
    mc.ricalcola_tutto()
    return mc
...
def test_03_fornitori(db, mat_ids, verbose=False):
    R.section("[3] FORNITORI — CRUD + assegnazioni + update/delete")
    nomi_forn = [f"FORN_{i:02d}" for i in range(10)]
    fids = []
    for nome in nomi_forn:
        fid, exc = safe(db.add_fornitore, nome)
        if isinstance(fid, int): fids.append(fid)
        else: R.fail(f"add_fornitore('{nome}')", exc)
    if len(fids) == 10: R.ok("Insert 10 fornitori: OK", verbose)
    else: R.fail(f"Insert: solo {len(fids)}/10")

    dup, _ = safe(db.add_fornitore, nomi_forn[0])
    if dup is False or dup is None: R.ok("Duplicato fornitore bloccato", verbose)
    else: R.fail("Duplicato fornitore NON bloccato")

    if mat_ids:
        campione30 = mat_ids[:min(30, len(mat_ids))]
        safe(db.assegna_materiali_a_fornitore, nomi_forn[0], campione30)
        scorte, _ = safe(db.get_scorte_per_fornitore, nomi_forn[0])
        scorte_ids = [r[0] for r in (scorte or [])]
        mancanti = [m for m in campione30 if m not in scorte_ids]
        if mancanti: R.fail(f"assegna_materiali: {len(mancanti)}/{len(campione30)} mancanti")
        else: R.ok(f"assegna_materiali_a_fornitore ({len(campione30)}): OK", verbose)

        campione20 = mat_ids[:min(20, len(mat_ids))]
        for mid in campione20:
            for forn in [nomi_forn[1], nomi_forn[2]]:
                safe(db.add_fornitore_a_materiale, mid, forn,
                     round(random.uniform(5,50),2), round(random.uniform(1,10),2), round(random.uniform(50,500),2))
        err_mf = sum(1 for mid in campione20 if not safe(db.get_fornitori_per_materiale, mid)[0])
        if err_mf: R.fail(f"add_fornitore_a_materiale: {err_mf}/{len(campione20)} senza fornitori")
        else: R.ok(f"add_fornitore_a_materiale (20 mat × 2 forn): OK", verbose)

        rows0, _ = safe(db.get_fornitori_per_materiale, campione20[0])
        if rows0:
            mf_id = rows0[0][0]
            safe(db.update_fornitore_materiale, mf_id, nomi_forn[1], 99.0, 5.0, 200.0)
            rows_a, _ = safe(db.get_fornitori_per_materiale, campione20[0])
            if any(r[0] == mf_id for r in (rows_a or [])): R.ok("update_fornitore_materiale: OK", verbose)
            else: R.fail("update_fornitore_materiale: mf_id scomparso")
            safe(db.delete_fornitore_materiale, mf_id)
            rows_d, _ = safe(db.get_fornitori_per_materiale, campione20[0])
            if any(r[0] == mf_id for r in (rows_d or [])): R.fail("delete_fornitore_materiale: ancora presente")
            else: R.ok("delete_fornitore_materiale: rimosso OK", verbose)

    safe(db.rename_fornitore, nomi_forn[5], nomi_forn[5]+"_REN")
    tutti, _ = safe(db.get_all_fornitori)
    nomi_t = [r[1] for r in (tutti or [])]
    if nomi_forn[5]+"_REN" in nomi_t and nomi_forn[5] not in nomi_t: R.ok("rename_fornitore: OK", verbose)
    else: R.fail("rename_fornitore: stato inconsistente")

    safe(db.add_fornitore, "COLL_A"); safe(db.add_fornitore, "COLL_B")
    res, _ = safe(db.rename_fornitore, "COLL_A", "COLL_B")
    if res: R.warn("rename su nome occupato → True")
    else: R.ok("rename su nome occupato → False", verbose)

    _, exc2 = safe(db.rename_fornitore, "NOME_INESISTENTE_ZZZ", "NUOVO")
    if exc2: R.fail("rename su inesistente → crash", exc2)
    else: R.ok("rename su inesistente → no crash", verbose)
    return nomi_forn

# =============================================================================
# 4. MAGAZZINO movimenti
# =============================================================================

def test_04_magazzino_movimenti(db, mat_ids, verbose=False):
    R.section(f"[4] MAGAZZINO — movimenti larga scala ({min(30,len(mat_ids))} materiali)")
    if not mat_ids: R.warn("Skip: nessun materiale"); return
    campione = mat_ids[:min(30, len(mat_ids))]
    for mid in campione:
        safe(db.add_fornitore_a_materiale, mid, "FORN_MAG", round(random.uniform(5,40),2), 5.0, 1000.0)

    err_bil = 0
    giacenze_attese = {}
    for mid in campione:
        carichi = [round(random.uniform(10,100),2) for _ in range(10)]
        scarichi = [round(random.uniform(1,15),2) for _ in range(5)]
        att = sum(carichi)
        for q in carichi: safe(db.registra_movimento, mid, "carico", q, fornitore_nome="FORN_MAG")
        for q in scarichi:
            safe(db.registra_movimento, mid, "scarico", q, fornitore_nome="FORN_MAG")
            att = max(att - q, 0.0)
        giacenze_attese[mid] = att
        g, exc = safe(db.get_giacenza_totale_materiale, mid)
        if exc or g is None or abs(g - att) > 0.05: err_bil += 1
    if err_bil: R.fail(f"Bilancio 10c+5s: {err_bil}/{len(campione)} discordanti")
    else: R.ok(f"Bilancio carico/scarico: tutti {len(campione)} concordi", verbose)

    mid_micro = campione[0]
    for _ in range(100):
        safe(db.registra_movimento, mid_micro, "carico", 0.001, fornitore_nome="FORN_MAG")
    g_micro, _ = safe(db.get_giacenza_totale_materiale, mid_micro)
    att_micro = giacenze_attese[mid_micro] + 0.1
    if g_micro is None or abs(g_micro - att_micro) > 0.05:
        R.fail(f"100 micro-carichi: atteso ~{att_micro:.3f}, trovato {g_micro}")
    else: R.ok("100 micro-carichi da 0.001: sommati correttamente", verbose)

    for mid in campione:
        safe(db.registra_movimento, mid, "scarico", 999999.0, fornitore_nome="FORN_MAG")
        g, _ = safe(db.get_giacenza_totale_materiale, mid)
        if g is not None and g < -0.001:
            R.fail(f"Giacenza sotto zero: mid={mid}, g={g}"); break
    else: R.ok("Scarico massivo: giacenza mai < 0", verbose)

    movs, _ = safe(db.get_movimenti_per_materiale, campione[0], 500)
    n_min = 10 + 5 + 100 + 1
    if movs is None or len(movs) < n_min:
        R.fail(f"get_movimenti: attesi ≥{n_min}, trovati {len(movs) if movs else 'None'}")
    else: R.ok(f"get_movimenti: {len(movs)} movimenti OK (limit=500)", verbose)

    _, exc0 = safe(db.registra_movimento, campione[0], "carico", 0.0, fornitore_nome="FORN_MAG")
    if exc0: R.fail("Quantità=0 → crash", exc0)
    else: R.ok("Quantità=0: no crash", verbose)
...
def test_05_magazzino_query(db, mat_ids, verbose=False):
    R.section("[5] MAGAZZINO — query, scorte, periodi")
    campione = mat_ids[:min(30, len(mat_ids))]
    scorte, exc = safe(db.get_scorte)
    if exc: R.fail("get_scorte crash", exc)
    else:
        ids_sc = [r[0] for r in (scorte or [])]
        trovati = sum(1 for m in campione if m in ids_sc)
        if trovati < len(campione): R.warn(f"get_scorte: {trovati}/{len(campione)} trovati")
        else: R.ok(f"get_scorte: tutti {len(campione)} presenti", verbose)
        if len(ids_sc) != len(set(ids_sc)): R.fail("get_scorte: ID duplicati")
        else: R.ok("get_scorte: nessun ID duplicato", verbose)

    sf, exc = safe(db.get_scorte_per_fornitore, "FORN_MAG")
    if exc: R.fail("get_scorte_per_fornitore crash", exc)
    elif not sf: R.warn("get_scorte_per_fornitore('FORN_MAG'): vuota")
    else: R.ok(f"get_scorte_per_fornitore: {len(sf)} record", verbose)

    sg, _ = safe(db.get_scorte_per_fornitore, "FORNITORE_INESISTENTE_XYZ_999")
    if sg: R.warn(f"get_scorte_per_fornitore(inesistente): {len(sg)} record")
    else: R.ok("get_scorte_per_fornitore(inesistente) → vuota", verbose)

    for ordine in ["giacenza_asc", "giacenza_desc", "nome_asc"]:
        sc, exc = safe(db.get_scorte_per_categoria, ordina_per=ordine)
        if exc: R.fail(f"get_scorte_per_categoria('{ordine}') crash", exc)
        else: R.ok(f"get_scorte_per_categoria('{ordine}'): {len(sc or [])} record", verbose)

    t_ini = (datetime.now() - timedelta(hours=2)).isoformat()
    t_fin = (datetime.now() + timedelta(hours=1)).isoformat()
    cons, exc = safe(db.get_consumi_periodo, t_ini, t_fin)
    if exc: R.fail("get_consumi_periodo (presente) crash", exc)
    elif not cons: R.warn("get_consumi_periodo (presente): vuota (attesi scarichi)")
    else: R.ok(f"get_consumi_periodo (presente): {len(cons)} materiali", verbose)

    t_old_i = (datetime.now() - timedelta(weeks=4)).isoformat()
    t_old_f = (datetime.now() - timedelta(weeks=2)).isoformat()
    cons_old, exc = safe(db.get_consumi_periodo, t_old_i, t_old_f)
    if exc: R.fail("get_consumi_periodo (vecchio) crash", exc)
    elif cons_old: R.warn(f"get_consumi_periodo (passato): {len(cons_old)} record (atteso 0)")
    else: R.ok("get_consumi_periodo (passato): vuota OK", verbose)
...
def test_06_categorie(db, mat_ids, verbose=False):
    R.section("[6] CATEGORIE — CRUD + relazioni materiali")
    cat_ids = []
    for i in range(15):
        cid, exc = safe(db.add_categoria, f"CAT_{i:02d}",
                        round(random.uniform(5,50),1), round(random.uniform(50,200),1),
                        round(random.uniform(200,1000),1), f"Note {i}")
        if isinstance(cid, int): cat_ids.append(cid)
        else: R.fail(f"add_categoria_{i}", exc)
    if len(cat_ids) == 15: R.ok("Insert 15 categorie: OK", verbose)
    else: R.fail(f"Insert: solo {len(cat_ids)}/15")

    dup, _ = safe(db.add_categoria, "CAT_00")
    if dup is False or dup is None: R.ok("Duplicato categoria bloccato", verbose)
    else: R.fail("Duplicato NON bloccato")

    err_upd = 0
    for cid in cat_ids[:8]:
        nn = f"CAT_UPD_{cid}"
        res, exc = safe(db.update_categoria, cid, nn, 12.0, 90.0, 450.0, "aggiornata")
        if exc or not res: err_upd += 1
        else:
            row, _ = safe(db.get_categoria_by_id, cid)
            if row is None or row[1] != nn: err_upd += 1
    if err_upd: R.fail(f"update_categoria: {err_upd}/8 discordanti")
    else: R.ok("update_categoria (8): OK", verbose)

    tutte, exc = safe(db.get_all_categorie)
    if exc: R.fail("get_all_categorie crash", exc)
    else:
        n = sum(1 for r in (tutte or []) if r[0] in cat_ids)
        if n != len(cat_ids): R.fail(f"get_all_categorie: attese {len(cat_ids)}, trovate {n}")
        else: R.ok(f"get_all_categorie: {n} categorie", verbose)

    err_gid = sum(1 for cid in cat_ids[:5] if safe(db.get_categoria_by_id, cid)[0] is None)
    if err_gid: R.fail(f"get_categoria_by_id: {err_gid}/5 None")
    else: R.ok("get_categoria_by_id: OK", verbose)

    if mat_ids:
        err_cnt = 0
        for cid in cat_ids[:4]:
            for j in range(5):
                safe(db.add_materiale, f"MC_{cid}_{j}", round(0.1*(j+1),1), float(j+1), categoria_id=cid)
            mats, _ = safe(db.get_materiali_per_categoria, cid)
            if mats is None or len(mats) != 5: err_cnt += 1
        if err_cnt: R.fail(f"get_materiali_per_categoria: {err_cnt}/4 errati")
        else: R.ok("get_materiali_per_categoria (5×4): OK", verbose)

        cid_cf = cat_ids[0]
        mid_cf, _ = safe(db.add_materiale, "MAT_CF_TEST2", 0.2, 10.0, categoria_id=cid_cf)
        if isinstance(mid_cf, int):
            safe(db.add_fornitore_a_materiale, mid_cf, "FORN_CF2", 8.0, 2.0, 100.0)
        rows_cf, exc = safe(db.get_materiali_per_categoria_con_fornitori, cid_cf)
        if exc: R.fail("get_materiali_per_categoria_con_fornitori crash", exc)
        elif rows_cf is None: R.fail("get_materiali_per_categoria_con_fornitori → None")
        else: R.ok(f"get_materiali_per_categoria_con_fornitori: {len(rows_cf)} record", verbose)

    if cat_ids:
        cid_del = cat_ids[-1]
        mid_surv, _ = safe(db.add_materiale, "MAT_SURV_CAT2", 0.1, 1.0, categoria_id=cid_del)
        safe(db.delete_categoria, cid_del)
        row_s, _ = safe(db.get_materiale_by_id, mid_surv)
        if row_s is None: R.fail("Delete categoria ha eliminato il materiale")
        else: R.ok("Delete categoria: materiale sopravvive", verbose)

    _, exc = safe(db.delete_categoria, 9999999)
    if exc: R.fail("Delete categoria inesistente → crash", exc)
    else: R.ok("Delete categoria inesistente → no crash", verbose)

    return cat_ids[:-1] if cat_ids else []

# =============================================================================
# 7. PREVENTIVI — creazione N scenari
# =============================================================================

def test_07_preventivi_create(db, N, verbose=False):
    R.section(f"[7] PREVENTIVI — {N} creazioni (6 profili diversificati)")
    profili = {
        "A_zero_mat": int(N*0.30),
        "B_1_mat":    int(N*0.20),
        "C_5_mat":    int(N*0.20),
        "D_15_mat":   int(N*0.15),
        "E_30_mat":   int(N*0.10),
    }
    profili["F_conica"] = N - sum(profili.values())
    pids = []; err_ins = {k:0 for k in profili}; err_rt = {k:0 for k in profili}

    def _ins(tag, n_mat, conica=False):
        mats = [_mc(conica=conica).to_dict() for _ in range(n_mat)]
        pid, exc = safe(db.add_preventivo, _prev_dict(materiali_utilizzati=mats))
        if exc or not isinstance(pid, int): err_ins[tag] += 1; return None
        prev, _ = safe(db.get_preventivo_by_id, pid)
        if prev is None or not isinstance(prev.get("materiali_utilizzati"), list): err_rt[tag] += 1
        elif len(prev["materiali_utilizzati"]) != n_mat: err_rt[tag] += 1
        return pid

    for _ in range(profili["A_zero_mat"]): pid=_ins("A_zero_mat",0); pids.append(pid) if pid else None
    for _ in range(profili["B_1_mat"]): pid=_ins("B_1_mat",1); pids.append(pid) if pid else None
    for _ in range(profili["C_5_mat"]): pid=_ins("C_5_mat",5); pids.append(pid) if pid else None
    for _ in range(profili["D_15_mat"]): pid=_ins("D_15_mat",15); pids.append(pid) if pid else None
    for _ in range(profili["E_30_mat"]): pid=_ins("E_30_mat",30); pids.append(pid) if pid else None
    for _ in range(profili["F_conica"]): pid=_ins("F_conica",random.randint(1,5),True); pids.append(pid) if pid else None

    for tag, cnt in profili.items():
        ne = err_ins[tag]; nr = err_rt[tag]
        thresh = max(1, int(cnt*0.02))
        if ne > thresh: R.fail(f"Profilo {tag}: {ne}/{cnt} insert falliti (>{thresh})")
        else: R.ok(f"Profilo {tag}: {cnt-ne}/{cnt} inseriti OK", verbose)
        if nr > thresh: R.fail(f"Profilo {tag}: {nr} round-trip discordanti")
        else: R.ok(f"Profilo {tag}: round-trip OK", verbose)

    R.info(f"Preventivi inseriti: {len(pids)}/{N}")
    return [p for p in pids if p is not None]

# =============================================================================
# 8. PREVENTIVI — update campi e save_preventivo
# =============================================================================

def test_08_preventivi_update(db, pids, verbose=False):
    R.section(f"[8] PREVENTIVI — update {min(100,len(pids))} + save_preventivo")
    if not pids: R.warn("Nessun preventivo, skip"); return
    campione = random.sample(pids, min(100, len(pids)))
    err_upd = 0
    for pid in campione:
        nc = f"CLI_{pid}"; no = f"ORD-{pid:05d}"
        nv = round(random.uniform(100,5000),2)
        safe(db.update_preventivo, pid, _prev_dict(nome_cliente=nc, numero_ordine=no, preventivo_finale=nv))
        prev, _ = safe(db.get_preventivo_by_id, pid)
        if prev is None: err_upd += 1
        elif prev.get("nome_cliente") != nc: err_upd += 1
        elif abs(prev.get("preventivo_finale",0) - nv) > 0.01: err_upd += 1
    if err_upd: R.fail(f"Update campi: {err_upd}/{len(campione)} discordanti")
    else: R.ok(f"Update {len(campione)} preventivi: OK", verbose)

    pid_mat = campione[0]
    nuovi = [_mc().to_dict() for _ in range(3)]
    safe(db.update_preventivo, pid_mat, _prev_dict(materiali_utilizzati=nuovi))
    prev_m, _ = safe(db.get_preventivo_by_id, pid_mat)
    if prev_m and isinstance(prev_m.get("materiali_utilizzati"), list) and len(prev_m["materiali_utilizzati"]) == 3:
        R.ok("Update materiali_utilizzati (3): OK", verbose)
    else: R.fail(f"Update materiali_utilizzati: attesi 3, trovati {len(prev_m.get('materiali_utilizzati',[]) if prev_m else [])}")

    _, exc = safe(db.update_preventivo, 9999999, _prev_dict())
    if exc: R.fail("update_preventivo ID inesistente → crash", exc)
    else: R.ok("update_preventivo ID inesistente → no crash", verbose)

    # save_preventivo upsert su esistente
    pid_s = campione[1]
    vus = round(random.uniform(200,3000),2)
    d_s = _prev_dict(preventivo_finale=vus); d_s["id"] = pid_s
    _, exc_s = safe(db.save_preventivo, d_s)
    if exc_s: R.fail("save_preventivo (upsert) crash", exc_s)
    else:
        prev_s, _ = safe(db.get_preventivo_by_id, pid_s)
        if prev_s and abs(prev_s.get("preventivo_finale",0) - vus) < 0.01:
            R.ok("save_preventivo (upsert esistente): aggiornato", verbose)
        else: R.warn("save_preventivo: valore non aggiornato")

    d_n = _prev_dict()
    if "id" in d_n: del d_n["id"]
    res_n, exc_n = safe(db.save_preventivo, d_n)
    if exc_n: R.fail("save_preventivo (nuovo) crash", exc_n)
    elif isinstance(res_n, int): R.ok(f"save_preventivo (nuovo): id={res_n}", verbose)
    else: R.warn(f"save_preventivo (nuovo) → {res_n!r}")

# =============================================================================
# 9. STORICO e RIPRISTINO
# =============================================================================

def test_09_storico_ripristino(db, pids, verbose=False):
    R.section("[9] PREVENTIVI — storico (20×10 update) e ripristino")
    if len(pids) < 20: R.warn(f"Solo {len(pids)} preventivi, skip"); return
    campione = random.sample(pids, 20)
    err_st = 0; err_ts = 0; step3_vals = {}

    for pid in campione:
        for j in range(10):
            v = round(random.uniform(50,4000),2)
            safe(db.update_preventivo, pid, _prev_dict(preventivo_finale=v))
        storico, _ = safe(db.get_storico_modifiche, pid)
        if storico is None or len(storico) < 10: err_st += 1; continue
        for e in storico:
            if "timestamp" not in e: err_ts += 1; break
        ts_list = [e.get("timestamp","") for e in storico]
        if ts_list != sorted(ts_list): R.warn(f"pid={pid}: timestamp non ordinati")
        if len(storico) >= 3: step3_vals[pid] = storico[2].get("preventivo_finale")

    if err_st: R.fail(f"Storico: {err_st}/20 incompleto (<10 entry)")
    else: R.ok("Storico 10 update × 20 prev: tutti ≥10 entry", verbose)
    if err_ts: R.fail(f"Storico: {err_ts} entry senza 'timestamp'")
    else: R.ok("Ogni entry ha 'timestamp'", verbose)

    con_mod, exc = safe(db.get_preventivi_con_modifiche)
    if exc: R.fail("get_preventivi_con_modifiche crash", exc)
    elif not con_mod: R.warn("get_preventivi_con_modifiche: vuota")
    else: R.ok(f"get_preventivi_con_modifiche: {len(con_mod)} record", verbose)

    err_rip = 0; ok_rip = 0
    for pid, v3 in list(step3_vals.items())[:10]:
        storico, _ = safe(db.get_storico_modifiche, pid)
        if not storico or len(storico) < 3: continue
        ts3 = storico[2]["timestamp"]
        res, exc = safe(db.ripristina_versione_preventivo, pid, ts3)
        if exc or not res: err_rip += 1; continue
        if v3 is not None:
            p, _ = safe(db.get_preventivo_by_id, pid)
            if p and abs(p.get("preventivo_finale",0) - v3) < 0.01: ok_rip += 1
            else: err_rip += 1
    if err_rip: R.fail(f"Ripristino passo 3: {err_rip} falliti")
    else: R.ok(f"Ripristino passo 3: {ok_rip} OK", verbose)

    res2, exc2 = safe(db.ripristina_versione_preventivo, campione[0], "2000-01-01T00:00:00")
    if exc2: R.fail("Ripristino ts inesistente → crash", exc2)
    elif res2: R.warn("Ripristino ts inesistente → True")
    else: R.ok("Ripristino ts inesistente → False/None: OK", verbose)

# =============================================================================
# 10. REVISIONI
# =============================================================================

def test_10_revisioni(db, pids, verbose=False):
    R.section(f"[10] PREVENTIVI — revisioni ({min(30,len(pids))} orig × 5 rev)")
    campione = pids[:min(30, len(pids))] if len(pids) < 30 else random.sample(pids, 30)
    rev_map = {}; err_rev = 0

    for pid_orig in campione:
        rev_ids = []
        for nr in range(2,7):
            rid, exc = safe(db.add_revisione_preventivo, pid_orig, _prev_dict(n_rev=nr), f"Rev {nr}")
            if exc or not isinstance(rid, int): err_rev += 1
            else: rev_ids.append(rid)
        rev_map[pid_orig] = rev_ids
    if err_rev: R.fail(f"add_revisione_preventivo: {err_rev} falliti")
    else: R.ok(f"5 revisioni × {len(campione)}: OK", verbose)

    err_cnt = sum(1 for pid, rids in list(rev_map.items())[:15]
                  if (safe(db.get_revisioni_preventivo, pid)[0] or []) and
                     len(safe(db.get_revisioni_preventivo, pid)[0] or []) != 1+len(rids))
    if err_cnt: R.fail(f"get_revisioni_preventivo count: {err_cnt}/15 errati")
    else: R.ok("get_revisioni_preventivo: 1+5=6 per gruppo (15 camp)", verbose)

    latest, _ = safe(db.get_all_preventivi_latest)
    ids_l = {row[0] for row in (latest or [])}
    orig_exp = [p for p in campione if p in ids_l]
    if orig_exp: R.warn(f"get_all_preventivi_latest: {len(orig_exp)} originali esposti")
    else: R.ok(f"get_all_preventivi_latest: nessun originale esposto", verbose)

    ultime = {p: rids[-1] for p, rids in rev_map.items() if rids}
    in_l = sum(1 for rid in ultime.values() if rid in ids_l)
    if in_l < len(ultime)*0.9: R.fail(f"latest: solo {in_l}/{len(ultime)} ultime rev presenti")
    else: R.ok(f"latest: {in_l}/{len(ultime)} ultime rev presenti", verbose)

    pid_base = campione[0]; rev_check = rev_map.get(pid_base, [])
    safe(db.delete_preventivo_e_revisioni, pid_base)
    superstiti = [i for i in [pid_base]+rev_check if safe(db.get_preventivo_by_id, i)[0] is not None]
    if superstiti: R.fail(f"Cascade delete: {len(superstiti)} record ancora presenti")
    else: R.ok(f"Cascade delete: orig + {len(rev_check)} rev eliminati", verbose)

    _, exc = safe(db.add_revisione_preventivo, 9999999, _prev_dict(), "orphan")
    if exc: R.warn(f"add_revisione su ID inesistente → eccezione: {exc}")
    else: R.ok("add_revisione su ID inesistente → no crash", verbose)
    return campione[1:]

# =============================================================================
# 11. PREVENTIVI edge case
# =============================================================================

def test_11_preventivi_edge(db, verbose=False):
    R.section("[11] PREVENTIVI — valori estremi e campi speciali")
    casi = [
        ("pf_zero",    {"preventivo_finale": 0.0}),
        ("pf_max",     {"preventivo_finale": 999999.99}),
        ("unicode",    {"nome_cliente": "Ünïcödé 日本語 العربية"}),
        ("nome_lungo", {"nome_cliente": "C"*500}),
        ("ord_vuoto",  {"numero_ordine": ""}),
        ("desc_nl",    {"descrizione": "riga1\nriga2\ttab"}),
        ("misura_cust",{"misura": "CUSTOM/FORMATO-2.5"}),
        ("tutti_zero", {"preventivo_finale":0.0,"costi_accessori":0.0,"minuti_taglio":0.0}),
    ]
    for tag, extra in casi:
        pid, exc = safe(db.add_preventivo, _prev_dict(**extra))
        if exc or not isinstance(pid, int): R.fail(f"Edge '{tag}': insert fallito", exc); continue
        prev, _ = safe(db.get_preventivo_by_id, pid)
        if prev is None: R.fail(f"Edge '{tag}': not found"); continue
        ok_rt = all(
            (abs(prev.get(k,0)-v)<0.001 if isinstance(v,float) else prev.get(k)==v)
            for k,v in extra.items() if k in prev
        )
        if ok_rt: R.ok(f"Edge '{tag}': round-trip OK", verbose)
        else: R.fail(f"Edge '{tag}': round-trip discordante")

    for vl, v in [("NaN",float("nan")),("Inf",float("inf"))]:
        _, exc = safe(db.add_preventivo, _prev_dict(preventivo_finale=v))
        if exc: R.ok(f"preventivo_finale={vl} → eccezione gestita", verbose)
        else: R.warn(f"preventivo_finale={vl} accettato")

    pid_empty_upd, _ = safe(db.add_preventivo, _prev_dict())
    _, exc_eu = safe(db.update_preventivo, pid_empty_upd, {})
    if exc_eu: R.warn(f"update_preventivo({{}}) → eccezione ({type(exc_eu).__name__}: campi obbligatori mancanti)")
    else: R.ok("update_preventivo({}) → no crash", verbose)

    _, exc2 = safe(db.add_preventivo, {"nome_cliente":"Solo questo"})
    if exc2: R.ok("add_preventivo (dict incompleto) → eccezione gestita", verbose)
    else: R.warn("add_preventivo (dict incompleto) non crasha")
...
def test_12_preventivi_query(db, pids, verbose=False):
    R.section("[12] PREVENTIVI — query get_all e coerenza")
    tutti, exc = safe(db.get_all_preventivi)
    if exc: R.fail("get_all_preventivi crash", exc); return
    if tutti is None: R.fail("get_all_preventivi → None"); return
    R.ok(f"get_all_preventivi: {len(tutti)} record", verbose)

    latest, exc = safe(db.get_all_preventivi_latest)
    if exc or latest is None: R.fail("get_all_preventivi_latest crash/None", exc); return
    R.ok(f"get_all_preventivi_latest: {len(latest)} record", verbose)

    if len(latest) > len(tutti): R.fail(f"latest ({len(latest)}) > tutti ({len(tutti)})")
    else: R.ok("Cardinalità: latest ≤ tutti", verbose)

    all_ids = [row[0] for row in tutti]
    if len(all_ids) != len(set(all_ids)): R.fail(f"get_all: {len(all_ids)-len(set(all_ids))} ID duplicati")
    else: R.ok("get_all_preventivi: nessun ID duplicato", verbose)

    tutti_set = set(all_ids)
    orfani = [row[0] for row in latest if row[0] not in tutti_set]
    if orfani: R.fail(f"latest: {len(orfani)} ID orfani")
    else: R.ok("Tutti gli ID di latest in get_all", verbose)

    da_del = pids[:min(50,len(pids))]
    n_prima = len(tutti)
    for pid in da_del: safe(db.delete_preventivo_e_revisioni, pid)
    tutti2, _ = safe(db.get_all_preventivi)
    if tutti2 and len(tutti2) > n_prima:
        R.fail(f"Dopo delete {len(da_del)}: count aumentato ({n_prima} → {len(tutti2)})")
    else: R.ok(f"Dopo delete {len(da_del)}: count non aumentato ({len(tutti2 or [])})", verbose)

# =============================================================================
# 13. CALCOLI puri
# =============================================================================

def test_13_calcoli_puri(N_CALC, verbose=False):
    R.section(f"[13] MATERIALECALCOLATO — calcoli puri ({N_CALC} campioni)")
    err_df=err_mag=err_nan=err_idem=err_ser=err_neg=err_md=err_ms=0

    for _ in range(N_CALC):
        mc = _mc()
        att_df = mc.diametro + mc.spessore*mc.giri*2
        if abs(mc.diametro_finale - att_df) > 0.001: err_df += 1
        if mc.costo_totale > 1e-9 and abs(mc.maggiorazione/mc.costo_totale-1.1) > 0.002: err_mag += 1
        if not all(math.isfinite(v) for v in [mc.sviluppo,mc.costo_totale,mc.maggiorazione,mc.diametro_finale,mc.lunghezza_utilizzata]): err_nan += 1
        if mc.lunghezza_utilizzata < -0.001 or mc.scarto_mm2 < -0.001: err_neg += 1
        s1,c1 = mc.sviluppo, mc.costo_totale; mc.ricalcola_tutto()
        if abs(mc.sviluppo-s1)>1e-9 or abs(mc.costo_totale-c1)>1e-9: err_idem += 1
        try:
            d=mc.to_dict(); d2=json.loads(json.dumps(d))
            if abs(d2.get("diametro",0)-mc.diametro)>0.001: err_ser+=1
        except: err_ser+=1

    # Monotonia giri: più giri → diametro_finale maggiore (a parità di dia e spes)
    n_mono = min(20, max(N_CALC//10, 5))
    for _ in range(n_mono):
        mc1=_mc(dia=100.0,spes=0.25,lun=200.0,giri=2); mc2=_mc(dia=100.0,spes=0.25,lun=200.0,giri=4)
        if mc2.diametro_finale <= mc1.diametro_finale: err_md += 1
    # Monotonia spessore: più spessore → sviluppo maggiore
    # sviluppo = ((dia + giri*spes)*3.14)*giri + 5 → aumenta con spes
    for _ in range(n_mono):
        mct=_mc(spes=0.1,lun=200.0,giri=3,dia=100.0); mck=_mc(spes=0.5,lun=200.0,giri=3,dia=100.0)
        if mck.sviluppo <= mct.sviluppo: err_ms += 1

    for label, err, tot in [
        ("diametro_finale formula", err_df, N_CALC),
        ("maggiorazione=costo×1.1", err_mag, N_CALC),
        ("NaN/Inf", err_nan, N_CALC),
        ("lunghezza/scarto ≥0", err_neg, N_CALC),
        ("idempotenza ricalcola_tutto", err_idem, N_CALC),
        ("serializzazione JSON", err_ser, N_CALC),
        ("monotonia giri→df", err_md, n_mono),
        ("monotonia spes→sviluppo crescente", err_ms, n_mono),
    ]:
        if err: R.fail(f"{label}: {err}/{tot} errori")
        else: R.ok(f"{label}: {tot}/{tot} OK", verbose)

# =============================================================================
# 14. CALCOLI edge
# =============================================================================

def test_14_calcoli_edge(verbose=False):
    R.section("[14] MATERIALECALCOLATO — edge case")
    mc0=_mc(giri=0,lun=200.0,dia=100.0,spes=0.25)
    if not math.isfinite(mc0.sviluppo): R.fail(f"giri=0 → sviluppo non finito: {mc0.sviluppo}")
    else: R.ok(f"giri=0 → sviluppo={mc0.sviluppo} (finito)", verbose)

    # lun=0 → lunghezza_utilizzata=0 → costo_totale=0
    mc_l0=_mc(lun=0.0)
    if mc_l0.costo_totale!=0.0 or mc_l0.maggiorazione!=0.0:
        R.fail(f"lun=0: costo={mc_l0.costo_totale}, mag={mc_l0.maggiorazione}")
    else: R.ok("lun=0 → costo=0, mag=0", verbose)

    # dia=0: diametro_finale = 0 + spes*giri*2
    mc_d0=_mc(dia=0.0,giri=3,spes=0.25,lun=100.0)
    att_d0 = 0.0 + 0.25*3*2  # = 1.5
    if abs(mc_d0.diametro_finale-att_d0)>0.001: R.fail(f"dia=0: df={mc_d0.diametro_finale} (atteso {att_d0})")
    else: R.ok(f"dia=0 → df={att_d0}", verbose)

    # spes=0: diametro_finale = dia
    mc_s0=_mc(spes=0.0,dia=150.0,giri=4,lun=200.0)
    if abs(mc_s0.diametro_finale-150.0)>0.001: R.fail(f"spes=0: df={mc_s0.diametro_finale} (atteso 150.0)")
    else: R.ok("spes=0 → df=diametro", verbose)

    # giri=1: sviluppo = ((dia + 1*spes)*3.14)*1 + 5
    mc_g1=_mc(giri=1,lun=300.0,dia=100.0,spes=0.2)
    att_sv_g1 = ((100.0 + 1*0.2)*3.14)*1 + 5
    if abs(mc_g1.sviluppo-att_sv_g1)>0.01: R.fail(f"giri=1: sviluppo={mc_g1.sviluppo} (atteso {att_sv_g1:.3f})")
    else: R.ok(f"giri=1 → sviluppo={(att_sv_g1):.2f} (formula verificata)", verbose)

    mc_c=_mc(conica=True)
    if mc_c.stratifica<0: R.fail(f"is_conica: stratifica<0")
    else: R.ok(f"is_conica: stratifica≥0 ({mc_c.stratifica:.2f})", verbose)
    if mc_c.sezioni_coniche is None: R.fail("is_conica: sezioni_coniche=None")
    else: R.ok("is_conica: sezioni_coniche non None", verbose)

    mc_nc=_mc(conica=False); mc_nc.is_conica=False; mc_nc.ricalcola_tutto()
    if mc_nc.sezioni_coniche: R.warn(f"is_conica=False: sezioni_coniche non vuota")
    else: R.ok("is_conica=False → sezioni_coniche vuota", verbose)

    for orient in ["orizzontale","verticale","","unknown"]:
        mc_o=_mc(); mc_o.orientamento=orient
        try: mc_o.ricalcola_tutto(); R.ok(f"orientamento='{orient}': no crash", verbose)
        except Exception as e: R.fail(f"orientamento='{orient}' → crash: {e}")

# =============================================================================
# 15. PREVENTIVO MODEL aggregazioni
# =============================================================================

def test_15_preventivo_model(verbose=False):
    R.section("[15] PREVENTIVO MODEL — aggregazioni (200 preventivi)")
    err_sub=err_mag=err_fin=err_sca=err_dict=0
    for _ in range(200):
        p=Preventivo(); n_mat=random.randint(0,15)
        for _ in range(n_mat): p.aggiungi_materiale(_mc())
        ...
        p.ricalcola_tutto()
        if abs(p.subtotale-(p.costo_totale_materiali+p.tot_mano_opera+p.costi_accessori))>0.02: err_sub+=1
        if abs(p.maggiorazione_25-p.subtotale*0.25)>0.02: err_mag+=1
        if abs(p.preventivo_finale-(p.subtotale+p.maggiorazione_25))>0.02: err_fin+=1
        p.ricalcola_scarto_totale()
        if abs(p.scarto_totale_mm2-sum(m.scarto_mm2 for m in p.materiali_calcolati))>0.01: err_sca+=1
        d=p.to_dict()
        if not isinstance(d.get("materiali_utilizzati",[]),list): err_dict+=1
        elif len(d["materiali_utilizzati"])!=n_mat: err_dict+=1

    for label,err in [("subtotale",err_sub),("maggiorazione_25",err_mag),
                       ("preventivo_finale",err_fin),("scarto_totale_mm2",err_sca),
                       ("to_dict struttura",err_dict)]:
        if err: R.fail(f"{label}: {err}/200 discordanti")
        else: R.ok(f"{label}: 200/200 OK", verbose)

    p2=Preventivo(); mcs=[_mc() for _ in range(5)]
    for mc in mcs: p2.aggiungi_materiale(mc)
    p2.ricalcola_tutto()
    # costo_totale_materiali = Σ maggiorazione (non costo_totale!)
    att=sum(m.maggiorazione for m in mcs)
    if abs(p2.costo_totale_materiali-att)>0.01: R.fail(f"costo_totale_materiali: att={att:.4f} (Σmag), got={p2.costo_totale_materiali:.4f}")
    else: R.ok("costo_totale_materiali = Σ maggiorazione mc: OK", verbose)

# =============================================================================
# 16. PREVENTIVO MODEL edge
# =============================================================================

def test_16_preventivo_model_edge(verbose=False):
    R.section("[16] PREVENTIVO MODEL — edge case")
    p0=Preventivo(); p0.costi_accessori=50.0; p0.ricalcola_tutto()
    if p0.costo_totale_materiali!=0.0: R.fail(f"0 mat: ctm={p0.costo_totale_materiali}")
    else: R.ok("0 materiali: ctm=0", verbose)

    p30=Preventivo()
    for _ in range(30): p30.aggiungi_materiale(_mc())
    p30.ricalcola_tutto()
    if len(p30.materiali_calcolati)!=30: R.fail(f"30 mat: len={len(p30.materiali_calcolati)}")
    else: R.ok("30 materiali: tutti inseriti", verbose)
    if p30.aggiungi_materiale(_mc()): R.fail("31° accettato")
    else: R.ok("31° rifiutato", verbose)

    p30.rimuovi_materiale(15)
    if len(p30.materiali_calcolati)!=29: R.fail(f"Dopo rimozione idx=15: len={len(p30.materiali_calcolati)}")
    else: R.ok("Rimozione idx=15: len=29", verbose)
    if not p30.aggiungi_materiale(_mc()): R.fail("Dopo rimozione, aggiungi fallisce")
    else: R.ok("Dopo rimozione: 30° aggiunto OK", verbose)

    pa=Preventivo()
    for _ in range(5): pa.aggiungi_materiale(_mc())
    while pa.materiali_calcolati: pa.rimuovi_materiale(0)
    pa.ricalcola_tutto()
    if pa.costo_totale_materiali!=0.0 or pa.subtotale!=0.0:
        R.fail(f"Dopo rimozione tutti: ctm={pa.costo_totale_materiali}, sub={pa.subtotale}")
    else: R.ok("Rimozione tutti: totali azzerati", verbose)

    pb=Preventivo(); pb.costi_accessori=100000.0; pb.ricalcola_tutto()
    if abs(pb.subtotale-100000.0)>0.01: R.fail(f"costi_accessori=100000: sub={pb.subtotale}")
    else: R.ok("costi_accessori=100000: subtotale corretto", verbose)

    pt=Preventivo(); pt.aggiungi_materiale(_mc()); pt.ricalcola_tutto()
    d=pt.to_dict()
    for f in ["costo_totale_materiali","subtotale","maggiorazione_25","preventivo_finale","scarto_totale_mm2","tot_mano_opera"]:
        if f not in d: R.fail(f"to_dict: manca '{f}'"); break
        elif not isinstance(d[f],(int,float)): R.fail(f"to_dict: '{f}' non float")
    else: R.ok("to_dict: tutti i campi numerici presenti", verbose)

# =============================================================================
# 17. INTEGRITÀ cross-tabella
# =============================================================================

def test_17_integrita(db, mat_ids, pids, verbose=False):
    R.section("[17] INTEGRITÀ DATI — cross-tabella")
    if not mat_ids or not pids: R.warn("Dati insufficienti, skip"); return

    mid=mat_ids[0]; mc_t=_mc(); mc_t.materiale_id=mid
    pid_t, _=safe(db.add_preventivo, _prev_dict(materiali_utilizzati=[mc_t.to_dict()]))
    safe(db.delete_materiale, mid)
    ps, _=safe(db.get_preventivo_by_id, pid_t)
    if ps is None: R.fail("Delete materiale ha eliminato il preventivo")
    else: R.ok("Delete materiale: preventivo sopravvive", verbose)

    pid2, _=safe(db.add_preventivo, _prev_dict())
    if isinstance(pid2, int):
        with _sqlite3.connect(db.db_path) as conn:
            conn.execute("UPDATE preventivi SET materiali_utilizzati=? WHERE id=?",
                         ("{{CORROTTO_NON_JSON", pid2))
        p2, exc2=safe(db.get_preventivo_by_id, pid2)
        if exc2: R.fail("JSON corrotto → crash", exc2)
        elif p2 is None: R.fail("JSON corrotto → None")
        elif not isinstance(p2.get("materiali_utilizzati",[]), list): R.fail("JSON corrotto → non lista")
        else: R.ok("JSON corrotto: fallback lista vuota, no crash", verbose)

    tutti, _=safe(db.get_all_preventivi); latest, _=safe(db.get_all_preventivi_latest)
    if tutti and latest:
        if len(latest)<=len(tutti): R.ok(f"Cardinalità: tutti={len(tutti)} ≥ latest={len(latest)}", verbose)
        else: R.fail(f"latest ({len(latest)}) > tutti ({len(tutti)})")
        tutti_set={row[0] for row in tutti}
        orfani=[row[0] for row in latest if row[0] not in tutti_set]
        if orfani: R.fail(f"latest: {len(orfani)} ID orfani")
        else: R.ok("Tutti gli ID di latest in get_all", verbose)
    ...
    if len(mat_ids)>1:
        mid2=mat_ids[1]
        safe(db.add_fornitore_a_materiale, mid2, "FORN_INT2", 10.0, 1.0, 50.0)
        rows_b, _=safe(db.get_fornitori_per_materiale, mid2)
        if rows_b:
            mf=rows_b[0][0]; safe(db.delete_fornitore_materiale, mf)
            rows_a, _=safe(db.get_fornitori_per_materiale, mid2)
            if any(r[0]==mf for r in (rows_a or [])): R.fail("delete_fornitore_materiale: ancora presente")
            else: R.ok("delete_fornitore_materiale + verifica: OK", verbose)

    _, exc=safe(db.get_all_materiali)
    if exc: R.fail("get_all_materiali crash dopo delete categoria", exc)
    else: R.ok("get_all_materiali dopo delete categoria: no crash", verbose)
...
def test_18_robustezza(db, verbose=False):
    R.section("[18] ROBUSTEZZA — SQL injection, ID anomali")
    payloads=[
        ("nome_cliente","'; DROP TABLE preventivi; --"),
        ("numero_ordine",'" OR 1=1--'),
        ("descrizione","<script>alert('xss')</script>"),
        ("codice","1; UPDATE preventivi SET preventivo_finale=0;--"),
        ("misura","\\x00\\x01"),
    ]
    for campo, payload in payloads:
        pid, exc=safe(db.add_preventivo, _prev_dict(**{campo: payload}))
        if exc: R.fail(f"SQL inj '{campo}' → crash", exc)
        else:
            prev, _=safe(db.get_preventivo_by_id, pid)
            if prev and prev.get(campo)==payload: R.ok(f"SQL inj '{campo}': letterale", verbose)
            elif prev: R.warn(f"SQL inj '{campo}': letto diverso ({prev.get(campo)!r})")
            else: R.fail(f"SQL inj '{campo}': not found dopo insert")

    _, exc=safe(db.add_materiale, "'; DROP TABLE materiali;--", 0.2, 10.0)
    if exc: R.ok("SQL inj nome materiale → eccezione", verbose)
    else: R.ok("SQL inj nome materiale: no crash", verbose)

    _, exc=safe(db.add_fornitore, "'; DROP TABLE;--")
    if exc: R.ok("SQL inj fornitore → eccezione", verbose)
    else: R.ok("SQL inj fornitore: no crash", verbose)

    for bad in [0,-1,9999999,-9999]:
        row, exc=safe(db.get_preventivo_by_id, bad)
        if exc: R.fail(f"get_preventivo_by_id({bad}) → crash", exc)
        elif row is not None: R.warn(f"get_preventivo_by_id({bad}) → non None")
        else: R.ok(f"get_preventivo_by_id({bad}) → None", verbose)

    _, exc=safe(db.delete_preventivo_e_revisioni, 9999999)
    if exc: R.fail("delete_preventivo(inesistente) → crash", exc)
    else: R.ok("delete_preventivo(inesistente) → no crash", verbose)

    mid_t, _=safe(db.add_materiale, "MAT_ROB", 0.2, 5.0)
    if isinstance(mid_t,int):
        _, exc=safe(db.registra_movimento, mid_t, "pippo", 10.0)
        if exc: R.warn(f"tipo='pippo' → eccezione: {exc}")
        else: R.ok("registra_movimento tipo='pippo' → no crash", verbose)

    row, exc=safe(db.get_materiale_by_id, 9999999)
    if exc: R.fail("get_materiale_by_id(inesistente) → crash", exc)
    elif row is not None: R.warn("get_materiale_by_id(inesistente) → non None")
    else: R.ok("get_materiale_by_id(inesistente) → None", verbose)

# =============================================================================
# 19. METODI DB non coperti
# =============================================================================

def test_19_metodi_extra(db, mat_ids, verbose=False):
    R.section("[19] METODI DB — copertura completa metodi rimanenti")
    if mat_ids:
        row_e, _=safe(db.get_materiale_by_id, mat_ids[0])
        if row_e:
            r2, exc=safe(db.get_materiale_by_nome, row_e[1])
            if exc or r2 is None: R.fail(f"get_materiale_by_nome esistente: None", exc)
            elif r2[1]!=row_e[1]: R.fail("get_materiale_by_nome: nome discordante")
            else: R.ok("get_materiale_by_nome (esistente): OK", verbose)

    r_g, exc=safe(db.get_materiale_by_nome, "NOME_INESISTENTE_ZZZ_999")
    if exc: R.fail("get_materiale_by_nome(inesistente) crash", exc)
    elif r_g: R.warn("get_materiale_by_nome(inesistente) → non None")
    else: R.ok("get_materiale_by_nome(inesistente) → None", verbose)

    for i in range(3): safe(db.add_categoria, f"CAT_EXTRA19_{i}", 5.0, 50.0, 200.0, "")
    tutte, exc=safe(db.get_all_categorie)
    if exc: R.fail("get_all_categorie crash", exc)
    elif tutte is None: R.fail("get_all_categorie → None")
    else: R.ok(f"get_all_categorie: {len(tutte)} categorie", verbose)

    cid_cf, _=safe(db.add_categoria, "CAT_CF19", 5.0, 50.0, 200.0, "")
    if isinstance(cid_cf,int):
        mid_cf, _=safe(db.add_materiale, "MAT_CF19", 0.2, 10.0, categoria_id=cid_cf)
        if isinstance(mid_cf,int): safe(db.add_fornitore_a_materiale, mid_cf, "FORN_CF19", 8.0, 2.0, 100.0)
        rc, exc=safe(db.get_materiali_per_categoria_con_fornitori, cid_cf)
        if exc: R.fail("get_materiali_per_categoria_con_fornitori crash", exc)
        elif rc is None: R.fail("get_materiali_per_categoria_con_fornitori → None")
        else: R.ok(f"get_materiali_per_categoria_con_fornitori: {len(rc)} record", verbose)

    pid_us, _=safe(db.add_preventivo, _prev_dict())
    if isinstance(pid_us,int):
        vus=round(random.uniform(100,5000),2)
        dus=_prev_dict(preventivo_finale=vus); dus["id"]=pid_us
        safe(db.save_preventivo, dus)
        pus, _=safe(db.get_preventivo_by_id, pid_us)
        if pus and abs(pus.get("preventivo_finale",0)-vus)<0.01: R.ok("save_preventivo upsert: OK", verbose)
        else: R.warn("save_preventivo upsert: valore non aggiornato")

    dn=_prev_dict()
    if "id" in dn: del dn["id"]
    rn, exc=safe(db.save_preventivo, dn)
    if exc: R.fail("save_preventivo (nuovo) crash", exc)
    elif isinstance(rn,int): R.ok(f"save_preventivo (nuovo): id={rn}", verbose)
    else: R.warn(f"save_preventivo (nuovo) → {rn!r}")

    pid_mod, _=safe(db.add_preventivo, _prev_dict())
    if isinstance(pid_mod,int):
        for _ in range(3): safe(db.update_preventivo, pid_mod, _prev_dict())
        cm, exc=safe(db.get_preventivi_con_modifiche)
        if exc: R.fail("get_preventivi_con_modifiche crash", exc)
        elif cm is None: R.fail("get_preventivi_con_modifiche → None")
        else:
            ids_cm={r[0] if isinstance(r,(list,tuple)) else r.get("id") for r in cm}
            if pid_mod in ids_cm: R.ok(f"get_preventivi_con_modifiche: {len(cm)} record, prev presente", verbose)
            else: R.warn(f"get_preventivi_con_modifiche: {len(cm)} record, pid={pid_mod} non trovato")

    if mat_ids:
        mid_mf=mat_ids[0] if mat_ids else None
        if mid_mf:
            safe(db.add_fornitore_a_materiale, mid_mf, "FORN_MF19", 15.0, 3.0, 200.0)
            rows_mf, _=safe(db.get_fornitori_per_materiale, mid_mf)
            if rows_mf:
                mf2=rows_mf[0][0]
                _, exc=safe(db.update_fornitore_materiale, mf2, "FORN_MF19", 77.77, 4.0, 300.0)
                if exc: R.fail("update_fornitore_materiale crash", exc)
                else: R.ok("update_fornitore_materiale: OK", verbose)
                safe(db.delete_fornitore_materiale, mf2)
                rows_post, _=safe(db.get_fornitori_per_materiale, mid_mf)
                if any(r[0]==mf2 for r in (rows_post or [])): R.fail("delete_fornitore_materiale: ancora presente")
                else: R.ok("delete_fornitore_materiale: scomparso", verbose)

    for ordine in ["giacenza_asc","giacenza_desc","nome_asc"]:
        r, exc=safe(db.get_scorte_per_categoria, ordina_per=ordine)
        if exc: R.fail(f"get_scorte_per_categoria('{ordine}') crash", exc)
        else: R.ok(f"get_scorte_per_categoria('{ordine}'): {len(r or [])} record", verbose)
...
def main():
    parser = argparse.ArgumentParser(description="RCS-App Diagnostic Bot v2")
    parser.add_argument("--quick", action="store_true",
                        help="Campione ridotto (N_MAT=15, N_PREV=50, N_CALC=100)")
    ...
    print(f"\n{BOLD}{chr(9552)*68}{RESET}")
    print(f"{BOLD}  RCS-App DIAGNOSTIC BOT v2{RESET}  "
          f"{'(quick)' if args.quick else f'N_MAT={N_MAT} N_PREV={N_PREV} N_CALC={N_CALC}'}")
    print(f"{BOLD}{chr(9552)*68}{RESET}")
    print(f"{DIM}  DB: file temporaneo isolato (non tocca produzione){RESET}")
    print(f"{DIM}  Avvio: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}{RESET}\n")

    t0 = time.time()
    db = make_db()

    mat_ids = test_01_materiali_bulk(db, N_MAT, args.verbose)
    test_02_materiali_edge(db, args.verbose)
    test_03_fornitori(db, mat_ids, args.verbose)
    test_04_magazzino_movimenti(db, mat_ids, args.verbose)
    cat_ids = test_06_categorie(db, mat_ids, args.verbose)
    test_05_magazzino_query(db, mat_ids, args.verbose)
    pids = test_07_preventivi_create(db, N_PREV, args.verbose)
    test_08_preventivi_update(db, pids, args.verbose)
    test_09_storico_ripristino(db, pids, args.verbose)
    test_10_revisioni(db, pids, args.verbose)
    test_11_preventivi_edge(db, args.verbose)
    test_12_preventivi_query(db, pids, args.verbose)
    test_13_calcoli_puri(N_CALC, args.verbose)
    test_14_calcoli_edge(args.verbose)
    test_15_preventivo_model(args.verbose)
    test_16_preventivo_model_edge(args.verbose)
    test_17_integrita(db, mat_ids, pids, args.verbose)
    test_18_robustezza(db, args.verbose)
    test_19_metodi_extra(db, mat_ids, args.verbose)
